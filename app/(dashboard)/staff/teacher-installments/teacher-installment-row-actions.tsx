...
export function TeacherInstallmentRowActions({
  installmentId,
  teacherId,
  totalAmount,
  dueDate,
    ...
    <>
      ...
      >
        <form action={updateInstallmentAction} className="space-y-3">
          ...
          <h3 className="text-base font-semibold">تعديل قسط الراتب</h3>
          <div className="space-y-1">
            <Label htmlFor={`due-t-${installmentId}`}>تاريخ الاستحقاق</Label>
            <Input id={`due-t-${installmentId}`} name="dueDate" type="date" required defaultValue={dueDate} />
          </div>
          <div className="space-y-1">
            <Label htmlFor={`amt-t-${installmentId}`}>المبلغ</Label>
            <Input
              id={`amt-t-${installmentId}`}
              name="totalAmount"
              type="number"
              min="0.01"
              step="0.01"
      ...
      >
        <form action={createInstallmentAction} className="space-y-3">
          ...
          <input type="hidden" name="preserveStatus" value={preserveStatus} />
          <h3 className="text-base font-semibold">قسط راتب جديد لنفس المعلم</h3>
          <div className="space-y-1">
            <Label htmlFor={`new-due-t-${installmentId}`}>تاريخ الاستحقاق</Label>
            <Input id={`new-due-t-${installmentId}`} name="dueDate" type="date" required />
          </div>
          <div className="space-y-1">
            <Label htmlFor={`new-amt-t-${installmentId}`}>المبلغ</Label>
            <Input
              id={`new-amt-t-${installmentId}`}
              name="totalAmount"
              type="number"
              min="0.01"
              step="0.01"
      ...
      >
        <form action={deleteInstallmentAction} className="space-y-3">
          <input type="hidden" name="installmentId" value={installmentId} />
          <input type="hidden" name="preserveStatus" value={preserveStatus} />
          <h3 className="text-base font-semibold">حذف قسط الراتب</h3>
